...
function Quiz() {
  ...
  // Check answers on submission and calculate scores
  const handleSubmit = () => {
    ...
    navigate('/results', { state: { categoryScores: newScore, lowestCategory: categoryWithLowestScore } });
  };
  ...
  return (
    <div>
      ...
      {submitted && (
        <div>
          <h3>Results</h3>
          <p style={{ color: lowestCategory === "Python" ? "red" : "black" }}>Python Score: {score.Python} / 5</p>
          <p style={{ color: lowestCategory === "React" ? "red" : "black" }}>React Score: {score.React} / 5</p>
          <p style={{ color: lowestCategory === "AWS" ? "red" : "black" }}>AWS Score: {score.AWS} / 5</p>
          <p style={{ marginTop: "1rem", fontWeight: "bold", color: "red" }}>
            {lowestCategory ? `You need to work on ${lowestCategory}` : ""}
          </p>
        </div>
      )}
    </div>
